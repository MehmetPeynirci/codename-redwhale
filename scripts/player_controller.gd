...
func _physics_process(delta: float) -> void:
	...
	elif Input.is_action_just_pressed("move_jump") and not _is_crouching:
...
func _apply_headbob(delta: float) -> void:
	...
	if is_on_floor() and horizontal_speed > 0.1:
		...
		var bob_x := cos(_bob_time * 0.5) * headbob_amplitude * 0.45 * crouch_mul
...
func _update_camera_effects(delta: float, input_dir: Vector2) -> void:
	...
	elif Input.is_action_pressed("move_sprint") and is_on_floor() and input_dir.y < -0.1:
		target_fov = sprint_fov
	camera.fov = lerp(camera.fov, target_fov, min(1.0, fov_lerp_speed * delta))

	var target_roll := -input_dir.x * camera_tilt_strength + _mouse_sway.x
	camera.rotation.z = lerp(camera.rotation.z, target_roll, min(1.0, 10.0 * delta))
	camera.rotation.x = lerp(camera.rotation.x, -_mouse_sway.y, min(1.0, 10.0 * delta))
