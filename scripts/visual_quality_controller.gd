...
func _apply_environment_quality(env: Environment) -> void:
	...
	_safe_set(env, &"tonemap_exposure", 0.74)
	_safe_set(env, &"tonemap_white", 5.0)

	_safe_set(env, &"adjustment_enabled", true)
	_safe_set(env, &"adjustment_brightness", 0.78)
	_safe_set(env, &"adjustment_contrast", 1.12)
	_safe_set(env, &"adjustment_saturation", 0.82)
	_safe_set(env, &"ambient_light_color", Color(0.1, 0.12, 0.18, 1.0))
	_safe_set(env, &"ambient_light_energy", 0.22)

	_safe_set(env, &"fog_enabled", true)
	_safe_set(env, &"fog_density", 0.0145)
	_safe_set(env, &"fog_height", -0.28)
	_safe_set(env, &"fog_height_density", 0.26)
	_safe_set(env, &"fog_light_energy", 0.2)
	_safe_set(env, &"fog_light_color", Color(0.15, 0.19, 0.29, 1.0))

	if fast_start_quality:
		_safe_set(env, &"volumetric_fog_enabled", true)
		_safe_set(env, &"volumetric_fog_density", 0.018)
		...
		_safe_set(env, &"ssil_intensity", 1.08)
		_safe_set(env, &"ssil_sharpness", 0.86)
		_safe_set(env, &"sdfgi_enabled", false)
		_safe_set(env, &"ssr_enabled", true)
		_safe_set(env, &"ssr_max_steps", 56)
		_safe_set(env, &"ssr_depth_tolerance", 0.24)
	else:
		_safe_set(env, &"volumetric_fog_enabled", true)
		_safe_set(env, &"volumetric_fog_density", 0.02)
		_safe_set(env, &"volumetric_fog_detail_spread", 10.0)
		_safe_set(env, &"volumetric_fog_anisotropy", 0.55)
	...
	_safe_set(env, &"glow_intensity", 0.23)
	_safe_set(env, &"glow_strength", 0.29)
	_safe_set(env, &"glow_bloom", 0.03)

func _apply_sun_quality(sun: DirectionalLight3D) -> void:
	_safe_set(sun, &"light_energy", 0.15)
	_safe_set(sun, &"light_indirect_energy", 0.2)
	_safe_set(sun, &"light_color", Color(0.44, 0.53, 0.74, 1.0))
	_safe_set(sun, &"shadow_enabled", true)
	_safe_set(sun, &"shadow_blur", 2.15)
	...
	_safe_set(sun, &"directional_shadow_max_distance", 170.0)
