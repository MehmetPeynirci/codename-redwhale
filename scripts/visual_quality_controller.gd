...
func _apply_environment_quality(env: Environment) -> void:
	...
	_safe_set(env, &"tonemap_exposure", 0.9)
	_safe_set(env, &"tonemap_white", 5.6)

	_safe_set(env, &"adjustment_enabled", true)
	_safe_set(env, &"adjustment_brightness", 0.9)
	_safe_set(env, &"adjustment_contrast", 1.08)
	_safe_set(env, &"adjustment_saturation", 0.86)
	_safe_set(env, &"ambient_light_color", Color(0.12, 0.15, 0.22, 1.0))
	_safe_set(env, &"ambient_light_energy", 0.3)

	_safe_set(env, &"fog_enabled", true)
	_safe_set(env, &"fog_density", 0.0118)
	_safe_set(env, &"fog_height", -0.22)
	_safe_set(env, &"fog_height_density", 0.2)
	_safe_set(env, &"fog_light_energy", 0.26)
	_safe_set(env, &"fog_light_color", Color(0.18, 0.22, 0.33, 1.0))

	if fast_start_quality:
		_safe_set(env, &"volumetric_fog_enabled", true)
		_safe_set(env, &"volumetric_fog_density", 0.014)
		...
		_safe_set(env, &"volumetric_fog_enabled", true)
		_safe_set(env, &"volumetric_fog_density", 0.0155)
		_safe_set(env, &"volumetric_fog_detail_spread", 10.0)
		_safe_set(env, &"volumetric_fog_anisotropy", 0.47)
	...
	_safe_set(env, &"glow_intensity", 0.25)
	_safe_set(env, &"glow_strength", 0.31)
	_safe_set(env, &"glow_bloom", 0.03)

func _apply_sun_quality(sun: DirectionalLight3D) -> void:
	_safe_set(sun, &"light_energy", 0.2)
	_safe_set(sun, &"light_indirect_energy", 0.26)
	_safe_set(sun, &"light_color", Color(0.5, 0.58, 0.78, 1.0))
	...
	_safe_set(sun, &"directional_shadow_max_distance", 180.0)
